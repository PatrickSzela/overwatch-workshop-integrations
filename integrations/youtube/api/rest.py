...
class YouTubeRestApi(ABC):
    ...
    def get_channel_id_from_handle(self, handle: str):
        ...
        match channels:
            case {"items": [{"id": str() as channel_id}, *_]}:
                return channel_id
            case _:
                raise RuntimeError(f"Failed to find a channel for handle {handle}")
    ...
    def get_live_stream_video_id(self, channel_id: str):
        ...
        playlist_items = self._call(
            ...
            params={"part": "contentDetails", "playlistId": f"UULV{channel_id[2:]}"},
        )

        match playlist_items:
            case {"items": [{"contentDetails": {"videoId": str() as video_id}}, *_]}:
                return video_id
            case _:
                raise RuntimeError(
    ...
    def get_live_streams(self, video_ids: list[str]):
        ...
        data = {"part": "snippet,liveStreamingDetails", "id": ",".join(video_ids)}
        ...
        for idx, item in enumerate(items):
            match item:
                case {
                    ...
                    "id": str() as video_id,
                    "liveStreamingDetails": {"activeLiveChatId": str() as chat_id},
                }:
